/**
 * /api/gst-otp-verify.js — Vercel Serverless Function
 *
 * Verifies GST OTP via Sandbox API.
 * Checks Firestore to prevent duplicate phone registrations.
 */
...
export default async function handler(req, res) {
  ...
  const rl = rateLimit(`otp-verify:${ip}`, 5, 10 * 60 * 1000);
  if (rl.limited) {
  ...
  try {
    ...
    // OTP is passed as query param; gstin + username in body
    ...
    console.error('GST OTP verify error:', err);
